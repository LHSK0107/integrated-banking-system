...
const UserInfo = () => {
  ...
  const onSubmit = (data) => {
    ...
    const verifyCode = `${getValues("emailCode1")}`
    sendVerifyCode
      ? axios
          .post("https://iam-api.site/api/email/verification", {
    ...
    <div className="form_container">
      <form className="userInfo_form flex flex_column justify_between align_center" onSubmit={handleSubmit(onSubmit)}>
        <div className="email_verify_wrap flex justify_between align_center" >
            <input type="text" placeholder="" {...register("emailCode1")} minLength={6} maxLength={6} />
        ...
        {errors.emailCode1 && <p>{errors.emailCode1?.message}</p>}
        <p>이메일을 확인할 수 없나요?</p>
